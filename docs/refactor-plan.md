...
**Status:** Implemented. `titleOverride` is now device-local state that is never written to or read from CloudKit. `invalidArguments` record-save failures are classified as permanent rejections, removed from the CloudKit queue, and persisted as locally blocked uploads that remain merge-protected until their content changes.
