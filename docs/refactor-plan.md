...
**Status:** Implemented. `titleOverride` is now device-local state that is never written to or read from CloudKit. Record-save failures use a fail-closed CloudKit policy: only known transient conditions remain retryable, while permission, entitlement, account-policy, configuration, quota, constraint, schema, asset, database, and unknown CloudKit failures are classified as permanent rejections. Non-CloudKit failures remain retryable because they do not prove the record payload is invalid. Permanent failures are removed from the automatic CloudKit queue and persisted as locally blocked uploads that remain merge-protected until their content changes or the user explicitly retries them.
...
**Implemented behavior:** `titleOverride` joins `isOpen` and `preferredFrame` as local device state: it is cleared on write, decoded as `nil`, preserved across remote replacement, and excluded from cloud-content comparisons so a locally retitled note no longer looks like a conflict on every sync. Separately, a `.permanentlyRejected` save classification removes the record from `CKSyncEngine` pending state and drops it from the in-flight note map. Partial failures are classified per record so permanent failures can be blocked without discarding recoverable `unknownItem` work. The note keeps `needsCloudUpload = true` plus a persisted upload block, so complete remote snapshots cannot delete or overwrite the only unsynced local copy. A complete remote snapshot with exactly the blocked content and modification date acknowledges that a prior save actually succeeded, adopts the remote revision/system fields, and clears the block; incomplete and reset snapshots cannot make that inference.

Blocked notes are excluded from automatic outgoing saves and retry scheduling; an actual content edit clears the block and queues the new payload. Explicit Retry, pull-to-refresh, and Sync Now actions include blocked notes in one bounded send batch so an app or schema fix can heal unchanged content without restoring an automatic loop. If CloudKit reports `unknownItem` for stale system fields, that batch gets exactly one scoped fresh-record retry. Whether the batch succeeds, fails transiently, or remains unresolved, its forced pending changes are removed before return so they cannot leak into a later automatic send. A manual retry requested during an active sync is coalesced into the next serialized pass instead of being dropped. Generic failed syncs no longer schedule a fixed five-second retry; retryable work remains dirty for CKSyncEngine or a later lifecycle/user sync trigger. Rejection logs include counts and a private error message without note content, titles, or note IDs.
